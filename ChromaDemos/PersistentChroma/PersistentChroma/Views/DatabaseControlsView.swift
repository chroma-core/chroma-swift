    ...
    var body: some View {
        GroupBox {
            VStack(spacing: 16) {
                ...
                ActionButton(title: "Delete Collection", disabled: !state.isPersistentInitialized || state.collections.isEmpty || state.persistentCollectionName.isEmpty) {
                    self.focused = false
                    try state.deleteCollection(name: state.persistentCollectionName)
                }
